...
// Note: Either ROOT_PASSWORD or SSH_KEY_PATH must be provided
const REQUIRED = ["VPS_IP", "GITHUB_REPO", "APP_DIR_NAME", "GITHUB_TOKEN"];
...
function load() {
  ...
  const cfg = {
    ...
    root_ssh_key: env.SSH_KEY_PATH,
    ssh_key_path: env.SSH_KEY_PATH,
    github_repo: env.GITHUB_REPO,
    github_token: env.GITHUB_TOKEN,
    app_dir_name: env.APP_DIR_NAME,
    app_port: env.APP_PORT || "3000",
    extra_ports: env.EXTRA_PORTS
      ? env.EXTRA_PORTS.split(",")
          .map((p) => p.trim())
          .filter(Boolean)
      : [],
  ...
  if (cfg.root_ssh_key) {
    cfg.root_ssh_key = cfg.root_ssh_key.replace("$HOME", os.homedir());
  }
  ...
  for (const key of REQUIRED) {
    ...
    if (
      !val ||
      PLACEHOLDERS.some((p) => val.toLowerCase().includes(p.toLowerCase()))
    ) {
      missing.push(key);
    }
  }

  // Check that either ROOT_PASSWORD or SSH_KEY_PATH is provided
  const hasRootPassword =
    cfg.root_password &&
    !PLACEHOLDERS.some((p) =>
      String(cfg.root_password).toLowerCase().includes(p.toLowerCase())
    );
  const hasRootSSHKey =
    cfg.root_ssh_key &&
    !PLACEHOLDERS.some((p) =>
      String(cfg.root_ssh_key).toLowerCase().includes(p.toLowerCase())
    );

  if (!hasRootPassword && !hasRootSSHKey) {
    missing.push("ROOT_PASSWORD or SSH_KEY_PATH");
  }
